...
//bool config::setting_temp_mdl( QpTempTableModel * mdl , const QString &tableName)
//{
//    if(! mdl)
//        return false;
...
bool config::setting_mdl( QpCmnSqlTableModel * mdl)
{
    if(! mdl)
        return false;
...
bool config::setting_view(QpTableViewWrapper *view)
{
    ...
    QpCmnSqlTableModel * mdl = qobject_cast<QpCmnSqlTableModel*>(view->model());
