...
void setStyle( QpApplication &app );
...
int main(int argc, char *argv[])
{
    ...
    QpApplication app(argc, argv);

    QpApplication::setApplicationName("qp_test_temp_table_model");
    QpApplication::setOrganizationName("BIT Ltd");
    ...
    {
        QMessageBox::critical( 0 , qp::dbg::error_,  QObject::tr("addDatabase returns false:\n error: %1").arg(sqlite.lastError().text()));
        return 0;
    }
    ...
    if (! sqlite.open())
    {
        QMessageBox::critical( 0 , qp::dbg::error_,  QObject::tr("database file is not opened:\n %1,\n error: %2").arg(sqlite.databaseName().arg(sqlite.lastError().text())));
...
//    {
        ...
        QObject::connect(w, SIGNAL(sig_changeLanguage(QString)),
                         (QpApplication*)(&app), SLOT(slot_change_language(QString)));
...
QString add_QpTableView()
{
    ...
    QString em1 = QString::number(hh);
    QString em2 = QString::number(hh*2);
    QString em3 = QString::number(hh*3);
    QString em4 = QString::number(hh*4);
...
void setStyle( QpApplication &app )
{
    ...
    QpTableViewWrapper::margin_hor = ww;

    QString st = add_QpTableView()+
            "\n"\
...
*/
            ...
            "   image:url(:view/img/checked.svg);\n"\
            "}\n"\
            "QCheckBox::indicator:!checked{\n"\
            "   image:url(:view/img/unchecked.svg);\n"\
            "}\n"\
