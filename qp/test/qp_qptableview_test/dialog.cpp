    ...
    mdl(0)
{
    ...
    qp::db::SECTIONS_TMPL matrix = prepare_matrix( *ui->txt2);


    view = new QpTableView ( this);

    view->init_template( matrix );
    //line1 << qp::db::SECTION_D( qp::db::LABEL_TYPE, Receipts_Dlg::cash , "НАЛ:");
...
bool Dialog::init_sql_model()
{
    ...
    qp::db::print_matrix( view->horizontalHeader()->get_matrix() , QString("init_sql_model ") ) ;
...
void Dialog::init_StandardItemModel()
{
    ...
    qp::db::SECTIONS_TMPL matrix = prepare_matrix( *ui->txt1);
...
{
    qp::db::SECTIONS_TMPL matrix =  prepare_matrix ( *ui->txt3);
    view->init_template( matrix );
    init_styles();
...
qp::db::SECTIONS_TMPL Dialog::prepare_matrix( const QPlainTextEdit & txt )
{
        ...
        foreach( QVariant var, lst1)
        {
            ...
            if( ok_int )
            {
                if( int_val >=0)
                {
                    ...
                    dd.logicalNumber = int_val;
                }
                else if( int_val <= qp::db::LABEL_FLD )
                {
                    dd.type = qp::db::LABEL_TYPE;
                    dd.logicalNumber = int_val;
...
void Dialog::slot_settinggs_edit( const QPoint& pp )
{
    ...
    qp::db::SECTION sect = view->index_At( pp );

    view->get_section_at( pp );

    qp::db::CELL cell = view->get_cell_at( pp );

    qp::db::CELL_STYLE defStl = view->get_section_default_style( cell.line, cell.xNum ) ;
