...
//--------------------------------------------------
int static_init_logging_System()
{
    qDebug("init logging_System");
    return 1;
}
static int someArray[] = {static_init_logging_System()} ;
...
bool logging_System::toStdOut_ = false;
...
void logging_System::duplicate(QFile * log_File)
{
    FILE *loggingStreamStdout;
    FILE *loggingStreamStderr;

    int fd = log_File->handle();
...
void logging_System::setLoggingCommands( int logTypes )
{
    types = (logging_System::LOG_TYPE)logTypes;
...
void logging_System::setLoggingToStdOut( bool std_out )
{
    toStdOut_ = std_out ;
}

const QString logging_System::getLogFile()
{

//    QString str = QString("%1\\%2").arg(getLogDir()).arg( logging_System::LOG_FILE_NAME );
    QString str = logFile->fileName();//  logging_System::logFilePath;

    qDebug() << "logging_System::getLogFile()" << str;

    return str;
}

//const QString logging_System::getLogDir()
//{
//    return logFile-> dirName;
//}
...
//        qDebug() << " logFilePath : " << logFilePath;
        ...
        logFile  = new QFile( dirName + "\\"+fileName );


        if( ! testLargeFileAndSave(logFile->fileName()) )
            ;
...
//                                  arg(logFilePath));
            ...
            qCritical()  << "can't open log file :  " << logFile->fileName();
...
bool logging_System::testLargeFileAndSave(QString &logFileName)
{
    bool res = false;
...
//                                   );
    ...
    if( sz > 100000)
    {
        ...
        QString remainToFile = info.absolutePath()+"\\"+ LOGS_HYSTORY_DIR+"\\log-" + fn;


//        QDir().mkpath(QFileInfo( logFile->fileName()).absolutePath());

        qDebug() << " to : " << remainToFile;
        res = file.rename( remainToFile );
