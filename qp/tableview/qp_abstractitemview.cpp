...
QpAbstractItemViewPrivate::QpAbstractItemViewPrivate()
    :
      ...
      selectionMode(QpAbstractItemView::ExtendedSelection),
      selectionBehavior(QpAbstractItemView::SelectItems),
      ...
      state(QpAbstractItemView::NoState),
      stateBeforeAnimation(QpAbstractItemView::NoState),
      editTriggers(QpAbstractItemView::DoubleClicked|QpAbstractItemView::EditKeyPressed),
      lastTrigger(QpAbstractItemView::NoEditTriggers),
      tabKeyNavigation(false),
      #ifndef QT_NO_DRAGANDDROP
      showDropIndicator(true),
      dragEnabled(false),
      dragDropMode(QpAbstractItemView::NoDragDrop),
      overwrite(false),
      dropIndicatorPosition(QpAbstractItemView::OnItem),
      ...
      verticalScrollMode(QpAbstractItemView::ScrollPerItem),
      horizontalScrollMode(QpAbstractItemView::ScrollPerItem),
...
void QpAbstractItemViewPrivate::init()
{
...
void QpAbstractItemViewPrivate::setHoverIndex(const QPersistentModelIndex &index)
{
    Q_Q(QpAbstractItemView);
    if (hover == index)
        return;
...
void QpAbstractItemViewPrivate::checkMouseMove(const QPersistentModelIndex &index)
{
...
/*!
    \class QpAbstractItemView

    \brief The QpAbstractItemView class provides the basic functionality for
    item view classes.

    \ingroup model-view


    QpAbstractItemView class is the base class for every standard view
    that uses a QAbstractItemModel. QpAbstractItemView is an abstract
    ...
    QpAbstractItemView::NoSelection.

    The QpAbstractItemView class is one of the \l{Model/View Classes}
    and is part of Qt's \l{Model/View Programming}{model/view framework}.

    The view classes that inherit QpAbstractItemView only need
    to implement their own view-specific functionality, such as
    drawing items, returning the geometry of items, finding items,
    etc.

    QpAbstractItemView provides common slots such as edit() and
    ...
    Some of QpAbstractItemView's functions are concerned with
    ...
    \snippet doc/src/snippets/code/src_gui_itemviews_QpAbstractItemView.cpp 0
    ...
    QpAbstractItemView provides a lot of protected functions. Some are
    concerned with editing, for example, edit(), and commitData(),
    whilst others are keyboard and mouse event handlers.

    \note If you inherit QpAbstractItemView and intend to update the contents
...
/*!
    \enum QpAbstractItemView::SelectionMode
...
/*!
    \enum QpAbstractItemView::SelectionBehavior
...
/*!
    \enum QpAbstractItemView::ScrollHint
...
/*!
    \enum QpAbstractItemView::EditTrigger
...
/*!
    \enum QpAbstractItemView::CursorAction
...
/*!
    \since 4.2
    \enum QpAbstractItemView::ScrollMode
...
/*!
    \fn QRect QpAbstractItemView::visualRect(const QModelIndex &index) const = 0
...
/*!
    \fn void QpAbstractItemView::scrollTo(const QModelIndex &index, ScrollHint hint) = 0
...
/*!
    \fn QModelIndex QpAbstractItemView::indexAt(const QPoint &point) const = 0
...
/*!
    \fn void QpAbstractItemView::activated(const QModelIndex &index)
...
/*!
    \fn void QpAbstractItemView::entered(const QModelIndex &index)
...
/*!
    \fn void QpAbstractItemView::viewportEntered()
...
/*!
    \fn void QpAbstractItemView::pressed(const QModelIndex &index)
...
/*!
    \fn void QpAbstractItemView::clicked(const QModelIndex &index)
...
/*!
    \fn void QpAbstractItemView::doubleClicked(const QModelIndex &index)
...
/*!
    \fn QModelIndex QpAbstractItemView::moveCursor(CursorAction cursorAction, Qt::KeyboardModifiers modifiers) = 0
...
/*!
    \fn int QpAbstractItemView::horizontalOffset() const = 0
...
/*!
    \fn int QpAbstractItemView::verticalOffset() const = 0
...
/*!
    \fn bool QpAbstractItemView::isIndexHidden(const QModelIndex &index) const
...
/*!
    \fn void QpAbstractItemView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags flags)
...
/*!
    \fn QRegion QpAbstractItemView::visualRegionForSelection(const QItemSelection &selection) const = 0
...
/*!
    \fn void QpAbstractItemView::update()
    \internal
*/
...
QpAbstractItemView::QpAbstractItemView(QWidget *parent)
    : QAbstractScrollArea(*(new QpAbstractItemViewPrivate), parent)
{
    d_func()->init();
}
...
/*!
    \internal
*/
QpAbstractItemView::QpAbstractItemView(QpAbstractItemViewPrivate &dd, QWidget *parent)
...
/*!
    ...
    \snippet doc/src/snippets/code/src_gui_itemviews_QpAbstractItemView.cpp 2
...
void QpAbstractItemView::setModel(QAbstractItemModel *model)
{
    ...
    {
        QP_ASSERT( disconnect(d->model, SIGNAL(destroyed()),
                              this, SLOT(_q_modelDestroyed())));

        QP_ASSERT( disconnect(d->model, SIGNAL(dataChanged(QModelIndex,QModelIndex)),
                              this, SLOT(dataChanged(QModelIndex,QModelIndex))));

        QP_ASSERT( disconnect(d->model, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                              this, SLOT(_q_headerDataChanged())));

        QP_ASSERT( disconnect(d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                              this, SLOT(rowsInserted(QModelIndex,int,int))));

        qDebug() << "QpAbstractItemView::setModel rowsInserted disconnect";

        QP_ASSERT( disconnect(d->model, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                              this, SLOT(rowsAboutToBeRemoved(QModelIndex,int,int))) );

        QP_ASSERT( disconnect(d->model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                              this, SLOT(_q_rowsRemoved(QModelIndex,int,int))));

        QP_ASSERT( disconnect(d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                              this, SLOT(_q_rowsInserted(QModelIndex,int,int))));

        QP_ASSERT( disconnect(d->model, SIGNAL(columnsAboutToBeRemoved(QModelIndex,int,int))),
                   this, SLOT(_q_columnsAboutToBeRemoved(QModelIndex,int,int)));

        QP_ASSERT( disconnect(d->model, SIGNAL(columnsRemoved(QModelIndex,int,int)),
                              this, SLOT(_q_columnsRemoved(QModelIndex,int,int))));

        QP_ASSERT( disconnect(d->model, SIGNAL(columnsInserted(QModelIndex,int,int)),
                              this, SLOT(_q_columnsInserted(QModelIndex,int,int))));

        QP_ASSERT( disconnect(d->model, SIGNAL(modelReset()), this, SLOT(reset()))==true);
        QP_ASSERT( disconnect(d->model, SIGNAL(layoutChanged()), this, SLOT(_q_layoutChanged()))==true);
    }
    ...
    if (d->model != QAbstractItemModelPrivate::staticEmptyModel())
    {
        ...
        if ( ! connect(d->model, SIGNAL(destroyed()),
                       this, SLOT(_q_modelDestroyed())))
            qCritical()<<"   connect error setModel destroyed/_q_modelDestroyed";

        if ( ! connect(d->model, SIGNAL(dataChanged(QModelIndex,QModelIndex)),
                       this, SLOT(dataChanged(QModelIndex,QModelIndex))))
            qCritical()<<"   connect error setModel dataChanged/dataChanged";

        if ( ! connect(d->model, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                       this, SLOT(_q_headerDataChanged())))
            qCritical()<<"   connect error setModel rowsInserted connect";


        if ( ! connect(d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                       this, SLOT(rowsInserted(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel rowsInserted/rowsInserted ";


        if ( ! connect(d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                       this, SLOT(_q_rowsInserted(QModelIndex,int,int))) )
            qCritical()<<"   connect error setModel rowsInserted/_q_rowsInserted ";

        if ( ! connect(d->model, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                       this, SLOT(rowsAboutToBeRemoved(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel rowsAboutToBeRemoved/rowsAboutToBeRemoved";

        if ( ! connect(d->model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_rowsRemoved(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel rowsRemoved/_q_rowsRemoved";

        if ( ! connect(d->model, SIGNAL(columnsAboutToBeRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_columnsAboutToBeRemoved(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel columnsAboutToBeRemoved/_q_columnsAboutToBeRemoved";

        if ( ! connect(d->model, SIGNAL(columnsRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_columnsRemoved(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel columnsRemoved/_q_columnsRemoved";

        if ( ! connect(d->model, SIGNAL(columnsInserted(QModelIndex,int,int)),
                       this, SLOT(_q_columnsInserted(QModelIndex,int,int))))
            qCritical()<<"   connect error setModel columnsInserted/_q_columnsInserted";

        if ( !(connect(d->model, SIGNAL(modelReset()), this, SLOT(reset()) )) )
            qCritical()<<"   connect error setModel modelReset/reset";

        if ( !connect(d->model, SIGNAL(layoutChanged()), this, SLOT(_q_layoutChanged())))
            qCritical()<<"   connect error setModel layoutChanged/_q_layoutChanged ";
...
/*!
     ...
     */
void QpAbstractItemView::setSelectionModel(QItemSelectionModel *selectionModel)
{
    // ### if the given model is null, we should use the original selection model
    QP_ASSERT(selectionModel);
    Q_D(QpAbstractItemView);

    if (selectionModel->model() != d->model) {
        qWarning("QpAbstractItemView::setSelectionModel() failed: "
...
/*!
     ...
     */
QItemSelectionModel* QpAbstractItemView::selectionModel() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         ...
         Any existing delegate will be removed, but not deleted. QpAbstractItemView
         ...
         \sa itemDelegate()
     */
void QpAbstractItemView::setItemDelegate(QAbstractItemDelegate *delegate)
{
    Q_D(QpAbstractItemView);
...
/*!
         ...
         \sa setItemDelegate()
     */
QAbstractItemDelegate *QpAbstractItemView::itemDelegate() const
{
...
/*!
         \reimp
     */
QVariant QpAbstractItemView::inputMethodQuery(Qt::InputMethodQuery query) const
{
...
/*!
         ...
         deleted. QpAbstractItemView does not take ownership of \a delegate.
     ...
     */
void QpAbstractItemView::setItemDelegateForRow(int row, QAbstractItemDelegate *delegate)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
QAbstractItemDelegate *QpAbstractItemView::itemDelegateForRow(int row) const
{
    Q_D(const QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::setItemDelegateForColumn(int column, QAbstractItemDelegate *delegate)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
QAbstractItemDelegate *QpAbstractItemView::itemDelegateForColumn(int column) const
{
    Q_D(const QpAbstractItemView);
...
/*!
         ...
         the given \a index.
     */
QAbstractItemDelegate *QpAbstractItemView::itemDelegate(const QModelIndex &index) const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::selectionMode
     ...
     */
void QpAbstractItemView::setSelectionMode(SelectionMode mode)
{
    Q_D(QpAbstractItemView);
    d->selectionMode = mode;
}

QpAbstractItemView::SelectionMode QpAbstractItemView::selectionMode() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::selectionBehavior
...
void QpAbstractItemView::setSelectionBehavior(QpAbstractItemView::SelectionBehavior behavior)
{
    Q_D(QpAbstractItemView);
    d->selectionBehavior = behavior;
}

QpAbstractItemView::SelectionBehavior QpAbstractItemView::selectionBehavior() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         ...
         \l{QpAbstractItemView::}{NoSelection}, the item is also be selected.
     ...
     */
void QpAbstractItemView::setCurrentIndex(const QModelIndex &index)
{
    Q_D(QpAbstractItemView);
...
/*!
         ...
         \sa setCurrentIndex()
     */
QModelIndex QpAbstractItemView::currentIndex() const
{
    Q_D(const QpAbstractItemView);
...
{
    qDebug() << "QpAbstractItemView::reset()";
    Q_D(QpAbstractItemView);
...
/*!
         ...
         \sa rootIndex()
     */
void QpAbstractItemView::setRootIndex(const QModelIndex &index)
{
    Q_D(QpAbstractItemView);
    if (index.isValid() && index.model() != d->model) {
        qWarning("QpAbstractItemView::setRootIndex failed : index must be from the currently set model");
        return;
    }
    d->root = index;
...
/*!
         ...
         \sa setRootIndex()
     */
QModelIndex QpAbstractItemView::rootIndex() const
{
...
/*!
     ...
     */
void QpAbstractItemView::selectAll()
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::edit(const QModelIndex &index)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::clearSelection()
{
    Q_D(QpAbstractItemView);
    if (d->selectionModel)
...
/*!
     ...
     */
void QpAbstractItemView::doItemsLayout()
{
    Q_D(QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::editTriggers
     ...
     */
void QpAbstractItemView::setEditTriggers(EditTriggers actions)
{
    Q_D(QpAbstractItemView);
    d->editTriggers = actions;
}

QpAbstractItemView::EditTriggers QpAbstractItemView::editTriggers() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \since 4.2
         \property QpAbstractItemView::verticalScrollMode
...
void QpAbstractItemView::setVerticalScrollMode(ScrollMode mode)
{
    Q_D(QpAbstractItemView);
    if (mode == d->verticalScrollMode)
        return;
    QModelIndex topLeft = indexAt(QPoint(0, 0)).idx;
    d->verticalScrollMode = mode;
    updateGeometries(); // update the scroll bars
    scrollTo(topLeft, QpAbstractItemView::PositionAtTop);
}

QpAbstractItemView::ScrollMode QpAbstractItemView::verticalScrollMode() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \since 4.2
         \property QpAbstractItemView::horizontalScrollMode
...
void QpAbstractItemView::setHorizontalScrollMode(ScrollMode mode)
{
    Q_D(QpAbstractItemView);
...
QpAbstractItemView::ScrollMode QpAbstractItemView::horizontalScrollMode() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \since 4.2
         \property QpAbstractItemView::dragDropOverwriteMode
         ...
         \sa dragDropMode
     */
void QpAbstractItemView::setDragDropOverwriteMode(bool overwrite)
{
    Q_D(QpAbstractItemView);
    d->overwrite = overwrite;
}

bool QpAbstractItemView::dragDropOverwriteMode() const
{
    Q_D(const QpAbstractItemView);
    return d->overwrite;
}
...
void QpAbstractItemView::setAutoScroll(bool enable)
{
    Q_D(QpAbstractItemView);
    d->autoScroll = enable;
}

bool QpAbstractItemView::hasAutoScroll() const
{
    Q_D(const QpAbstractItemView);
    return d->autoScroll;
}
...
/*!
         \since 4.4
         \property QpAbstractItemView::autoScrollMargin
     ...
     */
void QpAbstractItemView::setAutoScrollMargin(int margin)
{
    Q_D(QpAbstractItemView);
    d->autoScrollMargin = margin;
}

int QpAbstractItemView::autoScrollMargin() const
{
    Q_D(const QpAbstractItemView);
...
/*!
       \property QpAbstractItemView::tabKeyNavigation
       \brief whether item navigation with tab and backtab is enabled.
     */

void QpAbstractItemView::setTabKeyNavigation(bool enable)
{
    Q_D(QpAbstractItemView);
    d->tabKeyNavigation = enable;
}

bool QpAbstractItemView::tabKeyNavigation() const
{
    Q_D(const QpAbstractItemView);
...
#ifndef QT_NO_DRAGANDDROP
/*!
         \property QpAbstractItemView::showDropIndicator
...
void QpAbstractItemView::setDropIndicatorShown(bool enable)
{
    Q_D(QpAbstractItemView);
    d->showDropIndicator = enable;
}

bool QpAbstractItemView::showDropIndicator() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::dragEnabled
...
void QpAbstractItemView::setDragEnabled(bool enable)
{
    Q_D(QpAbstractItemView);
    d->dragEnabled = enable;
}

bool QpAbstractItemView::dragEnabled() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \since 4.2
         \enum QpAbstractItemView::DragDropMode
...
/*!
         \property QpAbstractItemView::dragDropMode
         \brief the drag and drop event the view will act upon

         \since 4.2
         \sa showDropIndicator dragDropOverwriteMode
     */
void QpAbstractItemView::setDragDropMode(DragDropMode behavior)
{
    Q_D(QpAbstractItemView);
...
QpAbstractItemView::DragDropMode QpAbstractItemView::dragDropMode() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::defaultDropAction
         \brief the drop action that will be used by default in QpAbstractItemView::drag()
         ...
         \sa showDropIndicator dragDropOverwriteMode
     */
void QpAbstractItemView::setDefaultDropAction(Qt::DropAction dropAction)
{
    Q_D(QpAbstractItemView);
    d->defaultDropAction = dropAction;
}

Qt::DropAction QpAbstractItemView::defaultDropAction() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::alternatingRowColors
     ...
     */
void QpAbstractItemView::setAlternatingRowColors(bool enable)
{
    Q_D(QpAbstractItemView);
...
bool QpAbstractItemView::alternatingRowColors() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         \property QpAbstractItemView::iconSize
         \brief the size of items' icons

         Setting this property when the view is visible will cause the
         items to be laid out again.
     */
void QpAbstractItemView::setIconSize(const QSize &size)
{
    Q_D(QpAbstractItemView);
    if (size == d->iconSize)
        return;
...
QSize QpAbstractItemView::iconSize() const
{
    Q_D(const QpAbstractItemView);
    return d->iconSize;
}
...
/*!
         \property QpAbstractItemView::textElideMode

         \brief the position of the "..." in elided text.

         The default value for all item views is Qt::ElideRight.
     */
void QpAbstractItemView::setTextElideMode(Qt::TextElideMode mode)
{
    Q_D(QpAbstractItemView);
    d->textElideMode = mode;
}

Qt::TextElideMode QpAbstractItemView::textElideMode() const
{
...
/*!
       \reimp
     */
bool QpAbstractItemView::focusNextPrevChild(bool next)
{
    Q_D(QpAbstractItemView);
...
/*!
       \reimp
     */
bool QpAbstractItemView::event(QEvent *event)
{
    Q_D(QpAbstractItemView);
    switch (event->type()) {
    case QEvent::Paint:
    ...
    case QEvent::Show:
        ...
        if (d->shouldScrollToCurrentOnShow) {
            ...
            const QModelIndex current = currentIndex();
            if (current.isValid() && (d->state == QpAbstractItemView::EditingState || d->autoScroll))
                scrollTo(current);
        }
        break;
...
/*!
         \fn bool QpAbstractItemView::viewportEvent(QEvent *event)
     ...
     */
bool QpAbstractItemView::viewportEvent(QEvent *event)
{
    Q_D(QpAbstractItemView);
    ...
    case QEvent::HoverEnter:
        d->setHoverIndex( indexAt(static_cast<QHoverEvent*>(event)->pos()).idx );
        break;
    ...
    case QEvent::WhatsThis:
    {
        ...
        const QModelIndex index = indexAt(he->pos()).idx;
...
/*!
     ...
     */
void QpAbstractItemView::mousePressEvent(QMouseEvent *event)
{
    Q_D(QpAbstractItemView);
    d->delayedAutoScroll.stop(); //any interaction with the view cancel the auto scrolling
    QPoint pos = event->pos();
    QPersistentModelIndex index = indexAt(pos).idx;
    ...
    else if (!indexAt(d->pressedPosition - offset).idx.isValid())
...
/*!
     ...
     */
void QpAbstractItemView::mouseMoveEvent(QMouseEvent *event)
{
    Q_D(QpAbstractItemView);
    QPoint topLeft;
...
/*!
     ...
     */
void QpAbstractItemView::mouseReleaseEvent(QMouseEvent *event)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::mouseDoubleClickEvent(QMouseEvent *event)
{
    Q_D(QpAbstractItemView);

    QModelIndex index = indexAt(event->pos()).idx;
    if (!index.isValid()
            || !d->isIndexEnabled(index)
            || (d->pressedIndex != index)) {
...
/*!
     ...
     */
void QpAbstractItemView::dragEnterEvent(QDragEnterEvent *event)
{
...
/*!
         ...
         \sa dropEvent() startDrag()
     */
void QpAbstractItemView::dragMoveEvent(QDragMoveEvent *event)
{
    //    Q_D(QpAbstractItemView);
    //    if (dragDropMode() == InternalMove
    //            && (event->source() != this || !(event->possibleActions() & Qt::MoveAction)))
...
/*!
         ...
         is being moved.
      */
bool QpAbstractItemViewPrivate::droppingOnItself(QDropEvent *event, const QModelIndex &index)
{
    Q_Q(QpAbstractItemView);
    Qt::DropAction dropAction = event->dropAction();
    if (q->dragDropMode() == QpAbstractItemView::InternalMove)
...
/*!
         \fn void QpAbstractItemView::dragLeaveEvent(QDragLeaveEvent *event)
     ...
     */
void QpAbstractItemView::dragLeaveEvent(QDragLeaveEvent *)
{
    Q_D(QpAbstractItemView);
    stopAutoScroll();
    setState(NoState);
...
/*!
         ...
         \sa startDrag()
     */
void QpAbstractItemView::dropEvent(QDropEvent *event)
{
    Q_D(QpAbstractItemView);
    if (dragDropMode() == InternalMove) {
...
/*!
         ...
         \internal
       */
bool QpAbstractItemViewPrivate::dropOn(QDropEvent *event, int *dropRow, int *dropCol, QModelIndex *dropIndex)
{
    //    Q_Q(QpAbstractItemView);
    //    if (event->isAccepted())
    //        return false;
    ...
    //            case QpAbstractItemView::AboveItem:
    //                row = index.row();
    //                col = index.column();
    //                index = index.parent();
    //                break;
    //            case QpAbstractItemView::BelowItem:
    //                row = index.row() + 1;
    //                col = index.column();
    //                index = index.parent();
    //                break;
    //            case QpAbstractItemView::OnItem:
    //            case QpAbstractItemView::OnViewport:
    //                break;
    //            }
    //        } else {
    //            dropIndicatorPosition = QpAbstractItemView::OnViewport;
    //        }
    //        *dropIndex = index;
    //        *dropRow = row;
    //        *dropCol = col;
...
QpAbstractItemView::DropIndicatorPosition
QpAbstractItemViewPrivate::position(const QPoint &pos, const QRect &rect, const QModelIndex &index) const
{
    QpAbstractItemView::DropIndicatorPosition r = QpAbstractItemView::OnViewport;
    if (!overwrite) {
        const int margin = 2;
        if (pos.y() - rect.top() < margin) {
            r = QpAbstractItemView::AboveItem;
        } else if (rect.bottom() - pos.y() < margin) {
            r = QpAbstractItemView::BelowItem;
        } else if (rect.contains(pos, true)) {
            r = QpAbstractItemView::OnItem;
        }
    } else {
    ...
    if (r == QpAbstractItemView::OnItem && (!(model->flags(index) & Qt::ItemIsDropEnabled)))
        r = pos.y() < rect.center().y() ? QpAbstractItemView::AboveItem : QpAbstractItemView::BelowItem;
...
/*!
     ...
     */
void QpAbstractItemView::focusInEvent(QFocusEvent *event)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::focusOutEvent(QFocusEvent *event)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::keyPressEvent(QKeyEvent *event)
{
    Q_D(QpAbstractItemView);
    d->delayedAutoScroll.stop(); //any interaction with the view cancel the auto scrolling
    ...
    if (newCurrent != oldCurrent && newCurrent.isValid() && d->isIndexEnabled(newCurrent)) {
        ...
        if (command != QItemSelectionModel::NoUpdate
            ...
            if (command & QItemSelectionModel::Current) {
                ...
                if (!indexAt(d->pressedPosition - d->offset()).idx.isValid())
...
void QpAbstractItemView::timerEvent(QTimerEvent *event)
{
    ...
    {
        if( debug_timers ) qDebug() << "QpAbstractItemView::timerEvent fetchMoreTimer  d->fetchMore() " << objectName();
        d->fetchMore();
    }
...
/*!
         \reimp
     */
void QpAbstractItemView::inputMethodEvent(QInputMethodEvent *event)
{
...
#ifndef QT_NO_DRAGANDDROP
/*!
         \enum QpAbstractItemView::DropIndicatorPosition
...
/*!
     ...
     */
QpAbstractItemView::DropIndicatorPosition QpAbstractItemView::dropIndicatorPosition() const
{
    Q_D(const QpAbstractItemView);
...
/*!
     ...
     */
QModelIndexList QpAbstractItemView::selectedIndexes() const
{
    Q_D(const QpAbstractItemView);
...
/*!
         ...
         QpAbstractItemView::AllEditTriggers.

         \sa closeEditor()
     */
bool QpAbstractItemView::edit(const QModelIndex &index, EditTrigger trigger, QEvent *event)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::updateEditorData()
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::updateEditorGeometries()
{
    qDebug() <<"QpAbstractItemView::updateEditorGeometries()";

    Q_D(QpAbstractItemView);

    if(d->editorIndexHash.isEmpty())
    {
        qDebug() <<"  d->editorIndexHash.isEmpty()";
        return;
    }

    QStyleOptionViewItemV4 option = d->viewOptionsV4();
    QpEditorIndexHash::iterator it = d->editorIndexHash.begin();
...
/*!
     ...
     */
void QpAbstractItemView::updateGeometries()
{
...
/*!
         \internal
     */
void QpAbstractItemView::verticalScrollbarValueChanged(int value)
{
    Q_D(QpAbstractItemView);
...
/*!
         \internal
     */
void QpAbstractItemView::horizontalScrollbarValueChanged(int value)
{
    Q_D(QpAbstractItemView);
...
/*!
         \internal
     */
void QpAbstractItemView::verticalScrollbarAction(int)
{
    //do nothing
}
...
/*!
         \internal
     */
void QpAbstractItemView::horizontalScrollbarAction(int)
{
    //do nothing
}
...
void QpAbstractItemView::closeEditor(QWidget *editor, QAbstractItemDelegate::EndEditHint hint)
{
    Q_D(QpAbstractItemView);
    ...
    case QAbstractItemDelegate::EditNextItem: {
        ...
        if (index.isValid()) {
            ...
            if (index.flags() & Qt::ItemIsEditable
                    && (!(editTriggers() & QpAbstractItemView::CurrentChanged)))
                edit(persistent);
        } break; }
...
/*!
         ...
         \sa closeEditor()
     */
void QpAbstractItemView::commitData(QWidget *editor)
{
    Q_D(QpAbstractItemView);
...
/*!
         ...
         \sa closeEditor()
     */
void QpAbstractItemView::editorDestroyed(QObject *editor)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::setHorizontalStepsPerItem(int steps)
{
    Q_UNUSED(steps)
    // do nothing
}
...
/*!
     ...
     */
int QpAbstractItemView::horizontalStepsPerItem() const
{
    return 1;
}
...
/*!
     ...
     */
void QpAbstractItemView::setVerticalStepsPerItem(int steps)
{
    Q_UNUSED(steps)
    // do nothing
}
...
/*!
     ...
     */
int QpAbstractItemView::verticalStepsPerItem() const
{
    return 1;
}
...
/*!
     ...
     */
void QpAbstractItemView::keyboardSearch(const QString &search)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
QSize QpAbstractItemView::sizeHintForIndex(const QModelIndex &index) const
{
    Q_D(const QpAbstractItemView);
...
/*!
     ...
     */
int QpAbstractItemView::sizeHintForRow(int row) const
{
    Q_D(const QpAbstractItemView);
...
/*!
         ...
         \sa sizeHintForRow()
     */
int QpAbstractItemView::sizeHintForColumn(int column) const
{
    Q_D(const QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::openPersistentEditor(const QModelIndex &index)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::closePersistentEditor(const QModelIndex &index)
{
    Q_D(QpAbstractItemView);
    if (QWidget *editor = d->editorForIndex(index).widget.data()) {
...
/*!
         ...
         \snippet doc/src/snippets/code/src_gui_itemviews_QpAbstractItemView.cpp 1
         ...
         \sa {Delegate Classes}
     */
void QpAbstractItemView::setIndexWidget(const QModelIndex &index, QWidget *widget)
{
    Q_D(QpAbstractItemView);
    if (!d->isIndexValid(index))
        return;
...
/*!
     ...
     */
QWidget* QpAbstractItemView::indexWidget(const QModelIndex &index) const
{
    Q_D(const QpAbstractItemView);
    if (d->isIndexValid(index))
        if (QWidget *editor = d->editorForIndex(index).widget.data())
...
/*!
     ...
     */
void QpAbstractItemView::scrollToTop()
{
...
/*!
     ...
     */
void QpAbstractItemView::scrollToBottom()
{
    Q_D(QpAbstractItemView);
    if (d->delayedPendingLayout) {
...
/*!
     ...
     */
void QpAbstractItemView::update(const QModelIndex &index)
{
    qDebug() << "QpAbstractItemView::update index" << index;
    Q_D(QpAbstractItemView);
...
/*!
         ...
         bottomRight.
     */
void QpAbstractItemView::dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight)
{
    // Single item changed
    Q_D(QpAbstractItemView);
    if (topLeft == bottomRight && topLeft.isValid())
    {
        const QpEditorInfo &editorInfo = d->editorForIndex(topLeft);
...
/*!
     ...
     */
void QpAbstractItemView::rowsInserted(const QModelIndex &index, int start, int end)
{
    qDebug() << "QpAbstractItemView::rowsInserted  not matter index:"<< index << " start"<< start << " end"<<end;
...
/*!
         ...
         \sa rowsInserted()
     */
void QpAbstractItemView::rowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    Q_D(QpAbstractItemView);
    ...
    QpEditorIndexHash::iterator i = d->editorIndexHash.begin();
    while (i != d->editorIndexHash.end()) {
        ...
        if (index.row() >= start && index.row() <= end && d->model->parent(index) == parent) {
            ...
            QpEditorInfo info = d->indexEditorHash.take(index);
...
/*!
         ...
         inclusive.
     */
void QpAbstractItemViewPrivate::_q_rowsRemoved(const QModelIndex &index, int start, int end)
{

    qDebug() << "QpAbstractItemViewPrivate::_q_rowsRemoved "<< index << " start"<< start << " end"<<end;
    ...
    q->setState(QpAbstractItemView::NoState);
#ifndef QT_NO_ACCESSIBILITY
#ifdef Q_WS_X11
    if (QAccessible::isActive()) {
...
/*!
         ...
         inclusive.
     */
void QpAbstractItemViewPrivate::_q_columnsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    Q_Q(QpAbstractItemView);

    q->setState(QpAbstractItemView::CollapsingState);

    // Ensure one selected item in single selection mode.
    QModelIndex current = q->currentIndex();
    if (current.isValid()
            && selectionMode == QpAbstractItemView::SingleSelection
    ...
    // Remove all affected editors; this is more efficient than waiting for updateGeometries() to clean out editors for invalid indexes
    QpEditorIndexHash::iterator it = editorIndexHash.begin();
    while (it != editorIndexHash.end()) {
        QModelIndex index = it.value();
        if (index.column() <= start && index.column() >= end && model->parent(index) == parent) {
            QWidget *editor = it.key();
            QpEditorInfo info = indexEditorHash.take(it.value());
...
/*!
         ...
         inclusive.
     */
void QpAbstractItemViewPrivate::_q_columnsRemoved(const QModelIndex &index, int start, int end)
{
    Q_UNUSED(index)
    Q_UNUSED(start)
    Q_UNUSED(end)
...
/*!
     ...
     */
void QpAbstractItemViewPrivate::_q_rowsInserted(const QModelIndex &index, int start, int end)
{
    Q_UNUSED(index)
    Q_UNUSED(start)
    Q_UNUSED(end)
...
/*!
     ...
     */
void QpAbstractItemViewPrivate::_q_columnsInserted(const QModelIndex &index, int start, int end)
{
    Q_UNUSED(index)
    Q_UNUSED(start)
    Q_UNUSED(end)
...
/*!
         \internal
     */
void QpAbstractItemViewPrivate::_q_modelDestroyed()
{
    model = QAbstractItemModelPrivate::staticEmptyModel();
    doDelayedReset();
}
...
/*!
     ...
     */
void QpAbstractItemViewPrivate::_q_layoutChanged()
{
    qDebug() << "QpAbstractItemViewPrivate::_q_layoutChanged()";
...
/*!
         ...
         \sa setSelection()
     */
void QpAbstractItemView::selectionChanged(const QItemSelection &selected,
...
/*!
         ...
         dataChanged() signal.
     */
void QpAbstractItemView::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    Q_D(QpAbstractItemView);
    QP_ASSERT(d->model);
...
/*!
     ...
     */
void QpAbstractItemView::startDrag(Qt::DropActions supportedActions)
{
    Q_D(QpAbstractItemView);
    ...
    if (indexes.count() > 0) {
        ...
        else if (supportedActions & Qt::CopyAction && dragDropMode() != QpAbstractItemView::InternalMove)
...
/*!
     ...
     */
QStyleOptionViewItem QpAbstractItemView::viewOptions() const
{
    Q_D(const QpAbstractItemView);
...
QStyleOptionViewItemV4 QpAbstractItemViewPrivate::viewOptionsV4() const
{
...
/*!
         ...
         \sa setState()
     */
QpAbstractItemView::State QpAbstractItemView::state() const
{
    Q_D(const QpAbstractItemView);
    return d->state;
}
...
/*!
         ...
         \sa state()
     */
void QpAbstractItemView::setState(State state)
{
    Q_D(QpAbstractItemView);
    d->state = state;
}
...
/*!
     ...
     */
void QpAbstractItemView::scheduleDelayedItemsLayout()
{
    Q_D(QpAbstractItemView);
    d->doDelayedItemsLayout();
}
...
/*!
     ...
     */
void QpAbstractItemView::executeDelayedItemsLayout()
{
    Q_D(QpAbstractItemView);
    d->executePostedLayout();
}
...
void QpAbstractItemView::setDirtyRegion(const QRegion &region)
{
    Q_D(QpAbstractItemView);
...
/*!
         ...
         If you implement scrollContentsBy() in a subclass of QpAbstractItemView, call this function
     ...
     */
void QpAbstractItemView::scrollDirtyRegion(int dx, int dy)
{
    Q_D(QpAbstractItemView);
...
/*!
         ...
         QpAbstractItemView, you should translate the area given by the paint event with
         the offset returned from this function.

         \sa scrollDirtyRegion(), setDirtyRegion()
     */
QPoint QpAbstractItemView::dirtyRegionOffset() const
{
    Q_D(const QpAbstractItemView);
...
/*!
       \internal
     */
void QpAbstractItemView::startAutoScroll()
{
...
/*!
       \internal
     */
void QpAbstractItemView::stopAutoScroll()
{
...
/*!
       \internal
     */
void QpAbstractItemView::doAutoScroll()
{
    ...
    QRect area = static_cast<QpAbstractItemView*>(d->viewport)->d_func()->clipRect(); // access QWidget private by bending C++ rules
...
#ifndef QT_NO_DRAGANDDROP
        d->dropIndicatorRect = QRect();
        d->dropIndicatorPosition = QpAbstractItemView::OnViewport;
#endif
        d->viewport->update();
    }
}
...
/*!
         ...
         \sa setSelection()
     */
QItemSelectionModel::SelectionFlags QpAbstractItemView::selectionCommand(const QModelIndex &index,
...
QItemSelectionModel::SelectionFlags QpAbstractItemViewPrivate::multiSelectionCommand(
...
QItemSelectionModel::SelectionFlags QpAbstractItemViewPrivate::extendedSelectionCommand(
        const QModelIndex &index, const QEvent *event) const
{
    ...
    if (event) {
        ...
        case QEvent::MouseButtonRelease: {
            ...
            if (((index == pressedIndex && selectionModel->isSelected(index))
                 || !index.isValid()) && state != QpAbstractItemView::DragSelectingState
    ...
    if (modifiers & Qt::ControlModifier)
        return QItemSelectionModel::Toggle|selectionBehaviorFlags();
    if (state == QpAbstractItemView::DragSelectingState) {
...
QpAbstractItemViewPrivate::contiguousSelectionCommand(const QModelIndex &index,
                                                      const QEvent *event) const
{
...
void QpAbstractItemViewPrivate::fetchMore()
{
    fetchMoreTimer.stop();

    if ( ! model->canFetchMore(root) )
    {
        //qDebug() << "QpAbstractItemViewPrivate::fetchMore()   canFetchMore false";
    ...
    //qDebug() << "QpAbstractItemViewPrivate::fetchMore()   canFetchMore true last"<<last;
    ...
    qDebug() << "QpAbstractItemViewPrivate::fetchMore()   rect"<<rect;

    if ( viewport->rect().intersects(rect) )
    {
        qDebug() << "QpAbstractItemViewPrivate::fetchMore()   intersects "<<true;
        model->fetchMore(root);
    }
}

bool QpAbstractItemViewPrivate::shouldEdit(QpAbstractItemView::EditTrigger trigger,
                                           const QModelIndex &index) const
{
    ...
    if (((flags & Qt::ItemIsEditable) == 0) || ((flags & Qt::ItemIsEnabled) == 0))
        return false;
    if (state == QpAbstractItemView::EditingState)
        return false;
    if (hasEditor(index))
        return false;
    if (trigger == QpAbstractItemView::AllEditTriggers) // force editing
        return true;
    if ((trigger & editTriggers) == QpAbstractItemView::SelectedClicked
            && !selectionModel->isSelected(index))
        return false;
    return (trigger & editTriggers);
}

bool QpAbstractItemViewPrivate::shouldForwardEvent(QpAbstractItemView::EditTrigger trigger,
                                                   const QEvent *event) const
{
    if (!event || (trigger & editTriggers) != QpAbstractItemView::AnyKeyPressed)
        return false;
...
bool QpAbstractItemViewPrivate::shouldAutoScroll(const QPoint &pos) const
{
    if (!autoScroll)
        return false;
...
void QpAbstractItemViewPrivate::interruptDelayedItemsLayout() const
{
    delayedLayout.stop();
...
QWidget *QpAbstractItemViewPrivate::editor(const QModelIndex &index,
...
void QpAbstractItemViewPrivate::updateEditorData(const QModelIndex &tl, const QModelIndex &br)
{
    ...
    QpIndexEditorHash::const_iterator it = indexEditorHash.constBegin();
...
/*!
     ...
     */
void QpAbstractItemViewPrivate::clearOrRemove()
{
#ifndef QT_NO_DRAGANDDROP
...
/*!
     ...
     */
void QpAbstractItemViewPrivate::checkPersistentEditorFocus()
{
    Q_Q(QpAbstractItemView);
...
const QpEditorInfo & QpAbstractItemViewPrivate::editorForIndex(const QModelIndex &index) const
{
    static QpEditorInfo nullInfo;

    QpIndexEditorHash::const_iterator it = indexEditorHash.find(index);
...
QModelIndex QpAbstractItemViewPrivate::indexForEditor(QWidget *editor) const
{
    QpEditorIndexHash::const_iterator it = editorIndexHash.find(editor);
...
void QpAbstractItemViewPrivate::removeEditor(QWidget *editor)
{
    QpEditorIndexHash::iterator it = editorIndexHash.find(editor);
    if (it != editorIndexHash.end())
    {
...
void QpAbstractItemViewPrivate::addEditor(const QModelIndex &index, QWidget *editor, bool isStatic)
{
    editorIndexHash.insert(editor, index);
    indexEditorHash.insert(index, QpEditorInfo(editor, isStatic));
}

bool QpAbstractItemViewPrivate::sendDelegateEvent(const QModelIndex &index, QEvent *event) const
{
    Q_Q(const QpAbstractItemView);
...
bool QpAbstractItemViewPrivate::openEditor(const QModelIndex &index, QEvent *event)
{
    Q_Q(QpAbstractItemView);
    ...
    q->setState(QpAbstractItemView::EditingState);
    w->show();
    w->setFocus();
...
QpItemViewPaintPairs QpAbstractItemViewPrivate::draggablePaintPairs(const QModelIndexList &indexes, QRect *r) const
{
    QP_ASSERT(r);
    Q_Q(const QpAbstractItemView);
    QRect &rect = *r;
    const QRect viewportRect = viewport->rect();
    QpItemViewPaintPairs ret;
...
QPixmap QpAbstractItemViewPrivate::renderToPixmap(const QModelIndexList &indexes, QRect *r) const
{
    QP_ASSERT(r);
    QpItemViewPaintPairs paintPairs = draggablePaintPairs(indexes, r);
...
void QpAbstractItemViewPrivate::selectAll(QItemSelectionModel::SelectionFlags command)
{
    if (!selectionModel)
        return;
...
bool QpAbstractItemViewPrivate::isPersistent(const QModelIndex &index) const
{
    //    /Q_D(const QAbstractItemModel);
    //QAbstractItemModelPrivate * aa = model->d_ptr.data();
    //d->data()
    //QAbstractItemModelPrivate * aa = static_cast<QAbstractItemModelPrivate *>( model->d_ptr.data() );
    //    /bool bbb = persistent.indexes.contains(index);

    return false; //???????
}


QModelIndexList QpAbstractItemViewPrivate::selectedDraggableIndexes() const
{
    Q_Q(const QpAbstractItemView);
