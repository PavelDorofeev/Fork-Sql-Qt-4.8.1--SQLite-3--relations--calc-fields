...
class LIB_PREFIX_QP QpTableView : public QpAbstractItemView
{
    Q_OBJECT
    ...
    bool init_template( const qp::db::SECTIONS_TMPL &matrix );//!!

    void setHorizontalHeader(QpHorHeaderView *header);

    void setVerticalHeader(QpVertHeaderView *header);

    const qp::db::CELL_STYLE get_section_default_style( int line, int numX ) const;
    ...
    qp::db::CELL get_cell_at( const QPoint & pos);
    ...
    void scrollTo(const QModelIndex &index, QpAbstractItemView::ScrollHint hint = QpAbstractItemView::EnsureVisible);
...
protected:
    ...
    QpTableView( QpTableViewPrivate &, const qp::db::SECTIONS_TMPL matrix, QWidget *parent);
