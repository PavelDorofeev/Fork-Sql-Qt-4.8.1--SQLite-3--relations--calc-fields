...
void QpAbstractItemView::setModel(QAbstractItemModel *model)
{
    ...
    {
        qp::meta::dis_cnct (d->model, SIGNAL(destroyed()),
                              this, SLOT(_q_modelDestroyed()));

        qp::meta::dis_cnct (d->model, SIGNAL(dataChanged(QModelIndex,QModelIndex)),
                              this, SLOT(dataChanged(QModelIndex,QModelIndex)));

        qp::meta::dis_cnct (d->model, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                              this, SLOT(_q_headerDataChanged()));

        qp::meta::dis_cnct (d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                              this, SLOT(rowsInserted(QModelIndex,int,int)));

        qDebug() << "QpAbstractItemView::setModel rowsInserted disconnect";

        qp::meta::dis_cnct (d->model, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                              this, SLOT(rowsAboutToBeRemoved(QModelIndex,int,int))) ;

        qp::meta::dis_cnct (d->model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                              this, SLOT(_q_rowsRemoved(QModelIndex,int,int)));

        qp::meta::dis_cnct (d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                              this, SLOT(_q_rowsInserted(QModelIndex,int,int)));

        qp::meta::dis_cnct ( d->model, SIGNAL(columnsAboutToBeRemoved(QModelIndex,int,int)),
                   this, SLOT(_q_columnsAboutToBeRemoved(QModelIndex,int,int))) ;

        qp::meta::dis_cnct (d->model, SIGNAL(columnsRemoved(QModelIndex,int,int)),
                              this, SLOT(_q_columnsRemoved(QModelIndex,int,int)));

        qp::meta::dis_cnct (d->model, SIGNAL(columnsInserted(QModelIndex,int,int)),
                              this, SLOT(_q_columnsInserted(QModelIndex,int,int)));

        qp::meta::dis_cnct (d->model, SIGNAL(modelReset()), this, SLOT(reset()));

        qp::meta::dis_cnct (d->model, SIGNAL(layoutChanged()), this, SLOT(_q_layoutChanged()));
    }
    ...
    if (d->model != QAbstractItemModelPrivate::staticEmptyModel())
    {
        ...
        qp::meta::cnct (d->model, SIGNAL(destroyed()),
                       this, SLOT(_q_modelDestroyed()));

        qp::meta::cnct (d->model, SIGNAL(dataChanged(QModelIndex,QModelIndex)),
                       this, SLOT(dataChanged(QModelIndex,QModelIndex)));

        qp::meta::cnct (d->model, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                       this, SLOT(_q_headerDataChanged()));

        qp::meta::cnct (d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                       this, SLOT(rowsInserted(QModelIndex,int,int)));
        ...
        qp::meta::cnct (d->model, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                       this, SLOT(rowsAboutToBeRemoved(QModelIndex,int,int)));

        qp::meta::cnct (d->model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_rowsRemoved(QModelIndex,int,int)));

        qp::meta::cnct (d->model, SIGNAL(columnsAboutToBeRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_columnsAboutToBeRemoved(QModelIndex,int,int)));

        qp::meta::cnct (d->model, SIGNAL(columnsRemoved(QModelIndex,int,int)),
                       this, SLOT(_q_columnsRemoved(QModelIndex,int,int)));

        qp::meta::cnct (d->model, SIGNAL(columnsInserted(QModelIndex,int,int)),
                       this, SLOT(_q_columnsInserted(QModelIndex,int,int)));

        qp::meta::cnct (d->model, SIGNAL(modelReset()), this, SLOT(reset()) ) ;

        qp::meta::cnct (d->model, SIGNAL(layoutChanged()), this, SLOT(_q_layoutChanged()));
...
/*!
     ...
     */
void QpAbstractItemView::setSelectionMode(QpAbstractItemView::SelectionMode mode)
{
    Q_D(QpAbstractItemView);
...
/*!
     ...
     */
void QpAbstractItemView::setEditTriggers( QpAbstractItemView::EditTriggers actions)
{
    Q_D(QpAbstractItemView);
...
void QpAbstractItemView::setVerticalScrollMode(QpAbstractItemView::ScrollMode mode)
{
    Q_D(QpAbstractItemView);
...
void QpAbstractItemView::setHorizontalScrollMode(QpAbstractItemView::ScrollMode mode)
{
    Q_D(QpAbstractItemView);
...
bool QpAbstractItemView::viewportEvent(QEvent *event)
{
    ...
    case QEvent::HoverEnter:
        d->setHoverIndex( index_At(static_cast<QHoverEvent*>(event)->pos()).idx );
        break;
    ...
    case QEvent::WhatsThis:
    {
        ...
        const QModelIndex index = index_At(he->pos()).idx;
...
void QpAbstractItemView::mousePressEvent(QMouseEvent *event)
{
    ...
    QPersistentModelIndex index = index_At(pos).idx;
    ...
    else if (!index_At(d->pressedPosition - offset).idx.isValid())
...
void QpAbstractItemView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    QPersistentModelIndex index = index_At(bottomRight).idx;
...
void QpAbstractItemView::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    QModelIndex index = index_At(event->pos()).idx;
    if (!index.isValid()
            || !d->isIndexEnabled(index)
            || (d->pressedIndex != index)) {
    ...
    if (newCurrent != oldCurrent && newCurrent.isValid() && d->isIndexEnabled(newCurrent)) {
        ...
        if (command != QItemSelectionModel::NoUpdate
            ...
            if (command & QItemSelectionModel::Current) {
                ...
                if (!index_At(d->pressedPosition - d->offset()).idx.isValid())
...
bool QpAbstractItemView::edit(const QModelIndex &index, QpAbstractItemView::EditTrigger trigger, QEvent *event)
{
    Q_D(QpAbstractItemView);
