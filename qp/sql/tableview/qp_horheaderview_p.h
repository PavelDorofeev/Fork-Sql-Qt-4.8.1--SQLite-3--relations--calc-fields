...
class  LIB_PREFIX_QP QpHorHeaderViewPrivate: public QpAbstractItemViewPrivate
{
...
public:
    ...
    QpHorHeaderViewPrivate()
        : state(NoState),
          ...
          grid(0),
          sectionNum_underCursor( qp::db::UNKNOWN_VALUE ),
          section_before_moved( qp::db::UNKNOWN_VALUE ),
          section_edge( qp::db::NONE ),
          xNum_sizing(-1),
          line_sizing(-1)
    {
...
public:
    ...
    QList < QList< qp::db::SECTION_D > > visual_matrix;   // lines / xNums
    // line0 : 0 1 2 2 5
    // line1 : 0 1 3 4 5

    QHash < int , qp::db::CELL_STYLE > sections_styles;   // int:sectionNumber

    QHash< int , qp::db::CELL_NODES > map; // int:sectionNumber field number into model (>=0) or label number (< -1)
    // -2,-3,.. labels cells

    QHash< int , int > interactive_sizes;

    QHash< QPair<int,int>, int> currSizes;
    ...
    qp::db::aaa sectionHandleAt(const QPoint &point);//!!
...
//    }
    ...
    QPair<int , int > line_xNum_beforeMoved; // line, xNum
    int section_before_moved;
    int sectionNum_underCursor;
    qp::db::SectionEdge section_edge;
