...
#include "qp/sql/tableview/qp_vertheaderview.h"
#include "qp/sql/tableview/qp_tableview_p.h"
...
const bool QpTableView::dbg_on = false;

const bool QpTableView::debug                   = QpTableView::dbg_on && false;
const bool QpTableView::debug_init              = QpTableView::dbg_on && false;
const bool QpTableView::debug_paint             = QpTableView::dbg_on && false;
const bool QpTableViewPrivate::debug_draw_cell  = QpTableView::dbg_on && false;
const bool QpTableView::debug_row_col_cnt_short = QpTableView::dbg_on && false;
const bool QpTableView::debug_row_col_cnt_full  = QpTableView::dbg_on && false;
const bool QpTableView::debug_geometry      = QpTableView::dbg_on &&false;
const bool QpTableView::debug_event         = QpTableView::dbg_on &&false;
const bool QpTableView::debug_paint_region  = QpTableView::dbg_on && false;
const bool QpTableView::debug_paint_border  = QpTableView::dbg_on && false;
const bool QpTableView::debug_resize        = QpTableView::dbg_on && false;
const bool QpTableView::debug_mdl_signals   = QpTableView::dbg_on && false;
const bool QpTableView::debug_selection     = QpTableView::dbg_on && false;
const bool QpTableView::debug_scroll        = QpTableView::dbg_on && false;

const bool QpTableViewPrivate::debug_mdl_signals    = QpTableView::dbg_on && false;
const bool QpTableViewPrivate::debug                = QpTableView::dbg_on && false;
const bool QpTableViewPrivate::debug_selection      = QpTableView::dbg_on && false;
const bool QpTableViewPrivate::debug_init           = QpTableView::dbg_on && false;
...
class QpTableCornerButton : public QAbstractButton
{
    Q_OBJECT
public:
    QpTableCornerButton(QWidget *parent) : QAbstractButton(parent) {}
...
void QpTableViewPrivate::init( )
{
    ...
    cornerWidget = new QpTableCornerButton(q);
    cornerWidget->setFocusPolicy(Qt::NoFocus);

    QP_ASSERT ( QObject::connect(cornerWidget, SIGNAL(clicked()), q, SLOT(selectAll())) == true);
                                       ...
                                       int sectionNum)
{
    ...
    //horizontalHeader->print_section_styles("QpTableViewPrivate::drawCellLabel");
    ...
    //    else
    //        qDebug() << q->tblName() << "::drawCellLabel stls not contains sectionNum:"<<sectionNum << opt.text<< opt.font << opt.rect << " state:"<< opt.state;
...
void QpTableViewPrivate::drawCell( QPainter *painter, const QStyleOptionViewItemV4 &option, const QModelIndex &index)
{
    ...
    if (selectionModel && selectionModel->isSelected( index))
    {
        if( debug_draw_cell ) qDebug() << "QpTableViewPrivate::drawCell State_Selected index" <<index ;
    ...
    if (index == hover)
    {
        ...
        if( debug_draw_cell ) qDebug() << "     State_MouseOver index" <<index ;
    }
    ...
    if (option.state & QStyle::State_Enabled)
    {
        ...
        if (( model->flags( index ) & Qt::ItemIsEnabled) == 0)
        {
            ...
            if( debug_draw_cell ) qDebug() << "     QPalette::Disabled index" <<index ;
    ...
    if( horizontalHeader->get_section_style( col , stl) )
    {
        painter->save();
    ...
    if (index == q->currentIndex())
    {
        ...
        if (focus)
        {
            ...
            if( debug_draw_cell ) qDebug() << "     State_HasFocus" << index;
        }
    }
      ...
      row_selected(-1)
{
    ...
    QP_ASSERT ( connect(this, SIGNAL( clicked(QModelIndex) ),
...
QpTableView::QpTableView(QpTableViewPrivate &dd,  const qp::db::SECTIONS_TMPL matrix, QWidget *parent)
...
//        qDebug() << "setModel _q_updateSpanInsertedColumns disconnect ok";
...
//        QP_ASSERT ( connect(model, SIGNAL(rowsInserted(QModelIndex,int,int)),
//                           this, SLOT(_q_updateSpanInsertedRows(QModelIndex,int,int))) == true);

//        QP_ASSERT ( connect(model, SIGNAL(columnsInserted(QModelIndex,int,int)),
//                           this, SLOT(_q_updateSpanInsertedColumns(QModelIndex,int,int))) == true);


//        qDebug() << "setModel _q_updateSpanInsertedColumns connect ok";

//        QP_ASSERT ( connect(model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
//                           this, SLOT(_q_updateSpanRemovedRows(QModelIndex,int,int))) == true);

//        QP_ASSERT ( connect(model, SIGNAL(columnsRemoved(QModelIndex,int,int)),
//                           this, SLOT(_q_updateSpanRemovedColumns(QModelIndex,int,int))) == true);
    }
...
void QpTableView::setRootIndex(const QModelIndex &index)
{
    ...
    if (index == d->root)
    {
        qDebug() << tblName()<<" 234 QpTableView::setRootIndex viewport()->update(); vpW:" << viewport()->width();
        viewport()->update();
        qDebug() << tblName()<<" 234 QpTableView::setRootIndex viewport()->update(); vpW:" << viewport()->width();
        return;
    }
...
void QpTableView::setHorizontalHeader(QpHorHeaderView *header )
{
    ...
    QP_ASSERT (connect(d->horizontalHeader,SIGNAL(sectionResized_Y()),
                      this, SLOT(linesInRowResized_Y())) == true); //!!

    QP_ASSERT (connect(d->horizontalHeader,SIGNAL(sectionResized_X(int)),
                      this, SLOT(xNumsResized_X(int)))== true); //!!;

    //    QP_ASSERT (connect(d->horizontalHeader,SIGNAL(sectionResized(int,int,int)),
    //                this, SLOT(columnResized(int,int,int))) == true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(sectionMoved(int,int,int)),
                      this, SLOT(columnMoved(int,int,int)))== true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(sectionCountChanged(int,int)),
                      this, SLOT(columnCountChanged(int,int)))== true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(sectionPressed(int)), this, SLOT(selectColumn(int)))== true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(sectionEntered(int)), this, SLOT(_q_selectColumn(int)))== true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(sectionHandleDoubleClicked(int)),
                      this, SLOT(resizeColumnToContents(int)))== true);

    QP_ASSERT (connect(d->horizontalHeader, SIGNAL(geometriesChanged()), this, SLOT(updateGeometries()))== true);
...
void QpTableView::setVerticalHeader(QpVertHeaderView *header)
{
    ...
    QP_ASSERT (connect(d->verticalHeader, SIGNAL(sectionResized(int,int,int)),
                      this, SLOT(rowResized(int,int,int)))== true );

    QP_ASSERT (connect(d->verticalHeader, SIGNAL(sectionMoved(int,int,int)),
                      this, SLOT(rowMoved(int,int,int)))== true );

    QP_ASSERT (connect(d->verticalHeader, SIGNAL(sectionCountChanged(int,int)),
                      this, SLOT(rowCountChanged(int,int)))== true );

    QP_ASSERT (connect(d->verticalHeader, SIGNAL(sectionPressed(int)), this, SLOT(selectRow(int))) == true );

    QP_ASSERT ( connect(d->verticalHeader, SIGNAL(sectionEntered(int)), this, SLOT( _q_selectRow(int) )) == true );

    QP_ASSERT (connect(d->verticalHeader, SIGNAL(sectionHandleDoubleClicked(int)),
                      this, SLOT(resizeRowToContents(int))) == true );

    QP_ASSERT (connect(d->verticalHeader, SIGNAL(geometriesChanged()), this, SLOT(updateGeometries())) == true );
}
        ...
        for ( int row = row_top; row <= row_bottom; row++)
        {
            ...
            for ( int line = 0 ; line < lines_count; ++line)
            {
                ...
                for (int numX = left_num; numX <= right_num; ++numX)
                {
                    ...
                    qp::db::SECTION_D dd = horizontalHeader->section_d_at_Num_x_line( numX , line );

                    int sectionNum = dd.logicalNumber;
                    ...
                    if( sectionNum <= qp::db::LABEL_FLD)
                    {
                        ...
                        qp::db::SECTION_D var = horizontalHeader->get_cell_at_line_xNum( line , numX );
                        ...
                        if( debug_row_col_cnt_full ) qDebug() << "               numX:"  << numX << "  label : "<< var.logicalNumber;
                        ...
                        //qDebug()<< tblName()<< option2.text<< " 344 stl : "<< stl.font;
                        ...
                        if ( horizontalHeader->get_section_style( sectionNum , stl) )
                        {
                            ...
                            d->drawCellLabel( &painter, option2, rowSel , row , sectionNum);
                        }
                        else
                            d->drawCellLabel( &painter, option2,  rowSel , row , sectionNum );
                    ...
                    if( betweenRowsBorder )
                    {
                        ...
                        if( sectionNum >=0 )
                        {
                            qp::db::CELL_NODES nd = horizontalHeader->get_logicalIdex_nodes( sectionNum );
...
qp::db::CELL QpTableView::get_cell_at( const QPoint & pos)
{
    Q_D(QpTableView);
    ...
    qp::db::SECTION_D dd =horizontalHeader()->get_cell_at_line_xNum( line , xNum);

    qp::db::CELL cell(line , xNum);

    cell.sectionNum = dd.logicalNumber;
...
void QpTableView::mousePressEvent(QMouseEvent *event)
{
    ...
    QModelIndex idx = index_At( pos ).idx;

    if( idx.column() == qp::db::LABEL_FLD )
    {
        if( selectionBehavior() == QpAbstractItemView::SelectRows )
        {
            selectRow( idx.row());
...
QModelIndex QpTableView::indexAt(const QPoint &point) const
{
    return QModelIndex();
}

qp::db::SECTION QpTableView::index_At(const QPoint &pos) const
{
    Q_D(const QpTableView);
    ...
    qp::db::SECTION section;

    if( col == qp::db::LABEL_FLD)
    {
        QModelIndex idx = d->model->index(row, 0, d->root);

        section.type = qp::db::LABEL_TYPE;
        section.idx = idx;
    ...
    case MoveLeft:
    {
        ...
        do {
            ...
            while (visualColumn >= 0)
            {
                ...
                if (r == -1 || c == -1 || ( d->isCellEnabled(r, c)))
                    break;
...
void QpTableView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags command)
{
    ...
    QModelIndex tl = index_At( QPoint( qMin(rect.left(), rect.right()), qMin(rect.top(), rect.bottom()))).idx;


    QModelIndex br = index_At( QPoint( qMax(rect.left(), rect.right()), qMax(rect.top(), rect.bottom()))).idx;
...
{
    if( debug_resize ) qDebug() << tblName()<< " QpTableView::updateGeometries() hor vpW:"<<horizontalHeader()->viewport()->width()<< " W:"<<horizontalHeader()->width()<< " rect:" << horizontalHeader()->rect();
...
void QpTableView::scrollTo(const QModelIndex &index, QpAbstractItemView::ScrollHint hint)
{
    Q_D(QpTableView);
...
void QpTableView::timerEvent(QTimerEvent *event)
{
    ...
    {
        if( debug_event || debug_resize ) qDebug() << "QpTableView::timerEvent xNumsResizeTimerID vpW:"<<viewport()->width();
        ...
        for (int i = d->xNumsToUpdate.size()-1; i >= 0; --i)
        {
            ...
            if( x == qp::db::UNKNOWN_VALUE )
                continue;

            rect |= QRect(x, 0, viewportWidth - x, viewportHeight);

            //if ( debug_timers ) qDebug() << "   xNumsResizeTimerID rect "<<rect;
        }
    ...
    if (event->timerId() == delayed_Repaint_tmr)
    {
        ...
        if( debug_mdl_signals )
        {
            qDebug() << tblName() <<" d->horizontalHeader->viewport()->geometry()"<< d->horizontalHeader->viewport()->geometry() ;
            qDebug() << tblName() <<" d->verticalHeader->viewport()->geometry()"<< d->verticalHeader->viewport()->geometry() ;
            qDebug() << tblName() <<" d->viewport()->geometry()"<< d->viewport->geometry() ;
        }
...
void QpTableView::resizeRowsToContents()
{
    ...
    qDebug() << "QpTableView::resizeRowsToContents()";

//    horizontalHeader()->resizeLinesHeights_ByHeaderTextSize();
...
void QpTableView::resizeColumnsToContents()
{
    ...
    d->horizontalHeader->setResizeMode( QpHorHeaderView::ResizeToContents );
...
{
    Q_D(const QpTableView);
    QP_ASSERT(d->isIndexValid(index));
...
void QpTableViewPrivate::correct_Style( const qp::db::CELL_STYLE &stls,
                                        QStyleOptionViewItemV4 &opt)
{
    ...
    //qDebug() << opt.text<< "opt.font " << opt.font << " stls.font"<<stls.font;

    if( opt.font != stls.font ||  opt.font.weight() != stls.font.weight())
    {
        opt.font = stls.font;
...
const qp::db::CELL_STYLE QpTableView::get_section_default_style( int line, int numX ) const
{
    Q_D( const QpTableView);
...
Qt::Alignment QpTableView::get_section_align( int line, int numX ) const
{
    ...
    //    const qp::db::CELL cell( line, numX);
...
bool QpTableView::set_section_align( int mdlfldNum, Qt::Alignment align )
{
    ...
    //horizontalHeader()->get stls[ mdlfldNum ].align = align;
...
bool QpTableView::init_template( const qp::db::SECTIONS_TMPL &matrix )
{
    //reset();
    ...
    //horizontalHeader()->print_section_styles( "QpTableView::init_template");
...
void QpTableViewPrivate::_q_updateSpanInsertedRows(const QModelIndex &parent, int start, int end)
{
    ...
    qDebug() << "QpTableViewPrivate::_q_updateSpanInsertedRows( start " << start << ", end " << end<<") nothing, verticalHeader->length()"<< verticalHeader->length();


    Q_UNUSED(parent)

    q->verticalHeader()->update();
    q->updateGeometries();
    q->delayed_Repaint();
...
{
    //if( debug_mdl_signals )
    qDebug() << "QpTableViewPrivate::_q_updateSpanInsertedColumns( start " << start << ", end " << end<<") nothing, verticalHeader->length()"<<verticalHeader->length();
...
void QpTableViewPrivate::_q_updateSpanRemovedRows(const QModelIndex &parent, int start, int end)
{
    ...
    qDebug() << "QpTableViewPrivate::_q_updateSpanRemovedRows( first " << start << ", last " << end<<") nothing sectionCount " << verticalHeader->count() << " verticalHeader->length()"<<verticalHeader->length();
    Q_UNUSED(parent)

    //verticalHeader->updateGeometries();
    q->updateGeometries();
    q->delayed_Repaint();
...
{
    qDebug() << "QpTableViewPrivate::_q_updateSpanRemovedColumns( first " << start << ", last " << end<<") nothing,  verticalHeader->length()"<<verticalHeader->length();
...
QAbstractItemModel *QpTableView::model() const
{
    return QpAbstractItemView::model();
}
