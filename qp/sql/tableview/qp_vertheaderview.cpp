...
#include "qp/sql/tableview/qp_vertheaderview_p.h"
...
const  bool QpVertHeaderView::debug_visualIndexAt_Y = false;
...
const  bool QpVertHeaderView::debug_mdl_signals = false;
...
const  bool QpVertHeaderViewPrivate::debug_mdl_signals = false;
      ...
      QpAbstractItemView(*new QpVertHeaderViewPrivate( Horizontal), parent)
{
    ...
    QP_ASSERT ( connect ( &Horizontal , SIGNAL(sig_sectionsTmplChanged()),
...
void QpVertHeaderView::setModel( QAbstractItemModel *mdl)
{
    ...
    if (d->model && d->model != QAbstractItemModelPrivate::staticEmptyModel())
    {
        ...
        if( ! disconnect(d->model, SIGNAL(rowsAboutToBeInserted(QModelIndex,int,int)),
                              this, SLOT(slot_rowsAboutToBeInserted(QModelIndex,int,int))) )
            qCritical() << "QpVertHeaderView::setModel disconnect ERROR rowsAboutToBeInserted/slot_rowsAboutToBeInserted";

        if( ! disconnect(d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
                                       this, SLOT(sectionsInserted(QModelIndex,int,int))) )

        qCritical() << "QpVertHeaderView::setModel disconnect ERROR rowsInserted/sectionsInserted";

        if( ! disconnect(d->model, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                                       this, SLOT(sectionsAboutToBeRemoved(QModelIndex,int,int))) )
            qCritical() << "QpVertHeaderView::setModel disconnect ERROR rowsAboutToBeRemoved/sectionsAboutToBeRemoved";

        if( ! disconnect(d->model, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                                       this, SLOT(_q_sectionsRemoved(QModelIndex,int,int))) )
            qCritical() << "QpVertHeaderView::setModel disconnect ERROR rowsRemoved/_q_sectionsRemoved";

        if( ! disconnect(d->model, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                                       this, SLOT(headerDataChanged(Qt::Orientation,int,int))) )
            qCritical() << "QpVertHeaderView::setModel disconnect ERROR headerDataChanged/headerDataChanged";

        if( ! disconnect(d->model, SIGNAL(layoutAboutToBeChanged()),
                                       this, SLOT(_q_layoutAboutToBeChanged())) )
            qCritical() << "QpVertHeaderView::setModel disconnect ERROR layoutAboutToBeChanged/_q_layoutAboutToBeChanged";
    }

    if (mdl && mdl != QAbstractItemModelPrivate::staticEmptyModel())
    {


        if( ! connect(mdl, SIGNAL(rowsAboutToBeInserted(QModelIndex,int,int)),
                                   this, SLOT(slot_rowsAboutToBeInserted(QModelIndex,int,int)))  )
            qCritical() << "QpVertHeaderView::setModel connect ERROR rowsAboutToBeInserted/slot_rowsAboutToBeInserted";


        if( ! connect(mdl, SIGNAL(rowsInserted(QModelIndex,int,int)),
                                   this, SLOT(sectionsInserted(QModelIndex,int,int)))  )
            qCritical() << "QpVertHeaderView::setModel connect ERROR rowsInserted/sectionsInserted";

        if( ! connect(mdl, SIGNAL(rowsAboutToBeRemoved(QModelIndex,int,int)),
                                   this, SLOT(sectionsAboutToBeRemoved(QModelIndex,int,int)))  )
            qCritical() << "QpVertHeaderView::setModel connect ERROR rowsAboutToBeRemoved/sectionsAboutToBeRemoved";

        if( ! connect(mdl, SIGNAL(rowsRemoved(QModelIndex,int,int)),
                                   this, SLOT(_q_sectionsRemoved(QModelIndex,int,int))) )
            qCritical() << "QpVertHeaderView::setModel connect ERROR rowsRemoved/_q_sectionsRemoved";

        if( ! connect(mdl, SIGNAL(headerDataChanged(Qt::Orientation,int,int)),
                                   this, SLOT(headerDataChanged(Qt::Orientation,int,int))) )
            qCritical() << "QpVertHeaderView::setModel connect ERROR headerDataChanged/headerDataChanged";

        if( ! connect(mdl, SIGNAL(layoutAboutToBeChanged()),
                                   this, SLOT(_q_layoutAboutToBeChanged()))  )
            qCritical() << "QpVertHeaderView::setModel connect ERROR layoutAboutToBeChanged/_q_layoutAboutToBeChanged";
    }

    d->state = QpVertHeaderViewPrivate::NoClear;
    QpAbstractItemView::setModel(mdl);
...
void QpVertHeaderView::slot_rowsAboutToBeInserted(const QModelIndex &index, int start ,int end)
{
    qDebug() << "QpVertHeaderView::slot_rowsAboutToBeInserted index"<<index<<start<<end;
...
int QpVertHeaderView::visualIndexAt_Y( int y ) const
{
    ...
    if ( yy > d->length )
    {
        if( debug_visualIndexAt_Y ) qDebug() << tblName() <<"???? QpVertHeaderView::visualIndexAt_Y((y:" << y<<"[yy:"<<yy<<"])) (d->offset:"<<d->offset<<"d->sectionCount:" << d->sectionCount << " d->length:" <<d->length << " return_visual:"<<qp::db::UNKNOWN_VALUE;
...
void QpVertHeaderView::moveSection(int from, int to)
{
    ...
    //    //QP_ASSERT(oldHeaderLength == length());
    //    // move sizes
    ...
    //        }
    //    }
    //    //QP_ASSERT(d->headerLength() == length());
    //    //QP_ASSERT(oldHeaderLength == length());
    //    //QP_ASSERT(d->logicalIndices.count() == d->sectionCount);
...
void QpVertHeaderView::reset()
{
    ...
    if(debug_init) qDebug() << tblName() <<" QpVertHeaderView::reset() section_Count " << d->sectionCount << " d->length" << d->length;
    ...
    //if(debug_init) qDebug() << tblName() <<" 2 QpVertHeaderView::reset() section_Count " << d->sectionCount << " d->length" << d->length;

    d->clear(); //!!

    //if(debug_init) qDebug() << tblName() <<" 3 QpVertHeaderView::reset() section_Count " << d->sectionCount << " d->length" << d->length;

    initializeSections();

    //if(debug_init) qDebug() << tblName() <<" 4 QpVertHeaderView::reset() section_Count " << d->sectionCount << " length"<<length();
                                         ...
                                         int logicalLast)
{
    ...
    if( rowHeight == qp::db::UNKNOWN_VALUE)
    {
        if( debug_mdl_signals ) qDebug() << tblName() << "QpVertHeaderView::sectionsInserted rowH == qp::db::UNKNOWN_VALUE";
        return ;
    }
    ...
    qDebug() << "emit sectionCountChanged("<<oldCount<<","<< count()<<")";
...
void QpVertHeaderView::paintEvent(QPaintEvent *e)
{
    ...
    int row_start = qp::db::UNKNOWN_VALUE;
    int row_end = qp::db::UNKNOWN_VALUE;
    ...
    row_start = (row_start == qp::db::UNKNOWN_VALUE ? 0 : row_start);
    row_end = (row_end == qp::db::UNKNOWN_VALUE ? count() - 1 : row_end);
...
*/
void QpVertHeaderView::rowsInserted(const QModelIndex &index, int start, int end)
{
    if ( debug_mdl_signals ) qDebug() << "QpVertHeaderView::rowsInserted does not matter index:"<<index<<start<<end;
    // do nothing
}
...
void QpVertHeaderView::scrollTo(const QModelIndex &, QpAbstractItemView::ScrollHint)
{
...
QModelIndex QpVertHeaderView::indexAt(const QPoint &p) const
{
    return  QModelIndex();
}

qp::db::SECTION QpVertHeaderView::index_At(const QPoint &) const
{
