...
#include <QSqlIndex>
#include "qp/sql/view/search_settings_dlg.h"
#include <QSqlDatabase>
#include <QIcon>
#include <QDoubleSpinBox>
#include "qp/gui/delegates/doubledelegate.h"
...
class Vertical_Header;
class QpTempSqlTableModel;
class QpAbstractTableModel;
...
class LIB_PREFIX_QP_SQL QpTableViewWrapper : public QTableView
{
    Q_OBJECT
public:
...
//    void mouseDoubleClickEvent(QMouseEvent *);
    ...
    const QpSqlRecord getBaseRec() const ;
    ...
    QpCmnSqlTableModel* model() const;
    virtual QpCmnSqlTableModel* Model() const;

    QpCmnSqlTableModel* qpAbstractModel() const;
    QpAbstractTableModel *qpMdl;
    ...
    virtual bool prepare_view( QpCmnSqlTableModel * mdl );

    Search_Settings_Dlg::FIND_SETTINGS find_settings;

    void setRelationDelegate(const QString & fldName);

    void setComboBoxDelegate(const QString & fldName, const QStringList &lst);

    void setComboBoxDelegate2(const QString & fldName, const QVariantList &lst);

    void setDateTimeDelegate(const QString & fldName, int fullDateTime=0);
    ...
    void setTempModel(QpTempSqlTableModel *newMdl);
...
private:
    ...
    void setModel( QpCmnSqlTableModel *model ); // now you have to use a prepare_view method ! Its incapsulates setTable.
