...
bool QpTableViewWrapper2::vrt_insertRow(int row)
{
    ...
    if ( QpTableViewWrapper::vrt_insertRow( row) )
    {
        ...
        int col = qpAbstractModel()->baseRec.indexOf("productName");
                                                ...
                                                bool &needsSubmitAfter)
{
    ...
    if( fldName == "productName")
    {
        ...
        QpAbstractTableModel* qpAbstractMdl = qpAbstractModel();

        int col = Model()->getBaseRec().indexOf("price");

        Model()->setData( Model()->index( row , col) , price );


        col = Model()->getBaseRec().indexOf("qty");
