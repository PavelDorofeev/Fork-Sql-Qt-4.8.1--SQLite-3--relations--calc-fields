...
#include "qp/db/view/qp_tableview_wrapper2.h"
#include "config.h"
#include "config2.h"
#include "logging_system/logging_system.h"
#include <QDateTime>
#include "qp/db/version_model.h"
    ...
    db(db_)
{
    ...
    mdl_purch = new QpSqlTableModel2(db , this);

    if( ! mdl_purch->prepare_mdl(tableName))
    {
    ...
    if( ! config2::setting_mdl2(mdl_purch)) //second level
    {
        QMessageBox::warning(this,
                             qp::dbg::error_,
...
void DialogTest::on_btn_save_clicked()
{
    ...
    {
        QMessageBox::critical(this , qp::dbg::error_ , tr("you should fill the table '%1'").arg(mdl_purch->tableName()));
        return;
    }
